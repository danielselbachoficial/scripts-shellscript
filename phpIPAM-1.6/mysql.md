...
Defina variáveis ​​para conexão de banco de dados em config.php

Abra a conexão mysql, crie banco de dados
mysql -u root -p

Copie as consultas SQL abaixo e cole-as no mysql

Finalizado ! Agora faça login com Admin/ipamadmin na página da web.

```bash
# Create phpipam database
# ------------------------------------------------------------
CREATE DATABASE phpipam;
```

```bash
# Set permissions for phpipam user
# ------------------------------------------------------------
CREATE USER 'phpipam'@'localhost' IDENTIFIED BY '<YOUR SECRET PASSWORD FROM config.php>';
